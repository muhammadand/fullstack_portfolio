...
<div class="max-w-6xl mx-auto px-6 py-10">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
        <h2 class="text-3xl font-bold text-slate-800">Daftar Kategori Portfolio</h2>

        <div class="flex gap-2">
            {{-- 🔍 Search Bar --}}
            <form method="GET" class="flex items-center gap-2">
                <input type="text" name="q" value="{{ request('q') }}" placeholder="Cari nama atau slug..." class="border border-gray-300 rounded-lg px-3 py-2 w-56 focus:ring focus:ring-blue-900/30 focus:border-blue-900 outline-none transition" />
                <button type="submit" class="bg-blue-900 text-white px-4 py-2 rounded-lg font-medium shadow hover:bg-blue-950 transition">
                    Cari
                </button>
            </form>

            {{-- ➕ Tambah Kategori --}}
            <a href="{{ route('portfolio-categories.create') }}" class="bg-blue-900 text-white px-5 py-2 rounded-lg font-medium shadow hover:bg-blue-950 transition">
                + Tambah Kategori
            </a>
        </div>
    </div>
            ...
            <tbody>
                ...
                <tr class="border-b hover:bg-gray-50 transition">
                    ...
                    <td class="py-3 px-4 text-center flex gap-2 justify-center">
                        <a href="{{ route('portfolio-categories.edit', $item->id) }}" class="px-3 py-1 bg-yellow-400 hover:bg-yellow-500 text-white rounded-lg text-xs font-semibold transition">
                            Edit
                        </a>
