...
<div class="min-h-screen bg-white py-6 px-4 sm:px-6 lg:px-8">
    <div class="max-w-6xl mx-auto">
        {{-- ✅ Header --}}
        <div class="mb-6">
            <h2 class="text-xl tracking-tight font-bold text-slate-900">Tambah Kategori Blog</h2>
            <p class="text-[13px] text-slate-500">Buat kategori baru untuk blog Anda</p>
        </div>
        ...
        {{-- ✅ Form --}}
        <form action="{{ route('blog-categories.store') }}" method="POST" class="bg-white p-6 rounded-xl shadow-sm border border-slate-200 flex flex-col gap-4">
            @csrf
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name') }}" class="w-full border border-gray-300 rounded-xl px-3 py-2 text-[13px] focus:ring-2 focus:ring-slate-800 focus:border-transparent transition-colors" required>
            </div>

            <div>
                <label for="description" class="block mb-1 text-[13px] font-medium tracking-wide text-slate-700">Deskripsi</label>
                <textarea name="description" id="description" rows="3" class="w-full border border-gray-300 rounded-xl px-3 py-2 text-[13px] focus:ring-2 focus:ring-slate-800 focus:border-transparent transition-colors resize-none">{{ old('description') }}</textarea>
            </div>

            <div>
                <label for="meta_description" class="block mb-1 text-[13px] font-medium tracking-wide text-slate-700">Meta Deskripsi</label>
                <input type="text" name="meta_description" id="meta_description" value="{{ old('meta_description') }}" class="w-full border border-gray-300 rounded-xl px-3 py-2 text-[13px] focus:ring-2 focus:ring-slate-800 focus:border-transparent transition-colors">
            </div>

            <div>
                <label for="display_order" class="block mb-1 text-[13px] font-medium tracking-wide text-slate-700">Urutan Tampilan</label>
                <input type="number" name="display_order" id="display_order" value="{{ old('display_order', 0) }}" class="w-full border border-gray-300 rounded-xl px-3 py-2 text-[13px] focus:ring-2 focus:ring-slate-800 focus:border-transparent transition-colors">
            </div>
