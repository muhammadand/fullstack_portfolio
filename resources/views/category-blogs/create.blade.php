...
<div class="max-w-3xl mx-auto px-6 py-10">
    ...
    <form action="{{ route('blog-categories.store') }}" method="POST" class="bg-white p-6 rounded-xl shadow-md flex flex-col gap-4">
        ...
        <div>
            ...
            <input type="text" name="name" id="name" value="{{ old('name') }}" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring focus:ring-purple-200" required>
        </div>

        <div>
            <label for="description" class="block mb-1 font-medium text-gray-700">Deskripsi</label>
            <textarea name="description" id="description" rows="3" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring focus:ring-purple-200">{{ old('description') }}</textarea>
        </div>

        <div>
            <label for="meta_description" class="block mb-1 font-medium text-gray-700">Meta Deskripsi</label>
            <input type="text" name="meta_description" id="meta_description" value="{{ old('meta_description') }}" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring focus:ring-purple-200">
        </div>

        <div>
            <label for="display_order" class="block mb-1 font-medium text-gray-700">Urutan Tampilan</label>
            <input type="number" name="display_order" id="display_order" value="{{ old('display_order', 0) }}" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring focus:ring-purple-200">
        </div>
